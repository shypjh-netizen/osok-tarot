...
[도입부 규칙]
- 게시물은 곧바로 리딩을 시작하지 말고, 독자가 데일리 타로 메시지를 받는다는 것을 알 수 있는 짧은 도입부로 시작합니다.
- 도입부에서 선택된 카드 이름("${card}")을 문장 안에 자연스럽게 한 번 밝힙니다. 카드 이름을 제목처럼 단독으로 표시하지 않습니다.
- 도입부는 1~2문장으로 짧게 작성하고 바로 본문 리딩으로 이어집니다.
- 도입부에서 카드 이름을 언급했다면 본문에서 다시 반복하지 않습니다.
- 카드 이름이 확인되지 않으면 임의로 만들어내지 않습니다.

피해야 할 도입부 표현:
- "이 글을 본 것은 우연이 아닙니다" / "운명처럼 이 글을 만나셨습니다"
- "지금 이 글이 당신에게 나타난 이유가 있습니다" / "우주가 당신에게 이 카드를 보냈습니다"
- "이 카드를 놓치면 안 됩니다"
- 독자의 불안이나 호기심을 과도하게 자극하는 운명론적 표현
- 팔로우·댓글·좋아요를 먼저 요구하는 문장
- 매일 동일하게 반복되는 고정 도입부

${postingPeriod === 'morning' ? `■ 오전 8시 30분 — 오늘의 흐름
오전 도입부 분위기: 하루를 시작하며 카드의 메시지를 가볍게 받아보는 분위기로 작성합니다.
도입부 예시 (매일 그대로 반복하지 말고 변형해서 사용):
- 오늘 아침의 카드는 '○○'예요. 이 카드가 전하는 메시지를 가볍게 받아가세요.
- 오늘 하루를 함께할 카드는 '○○'예요. 지금 필요한 메시지를 천천히 살펴볼게요.
- 오늘 아침 '○○' 카드가 찾아왔어요. 어떤 이야기를 전하는지 함께 볼까요?
...
리딩 구성:
도입부(1~2문장): 카드 이름을 자연스럽게 포함해 하루를 시작하는 분위기로 엽니다.
1문단: 독자가 오늘 느낄 수 있는 감정이나 상황을 짧게 묻습니다. (도입부와 비슷한 내용을 반복하지 않습니다.)
...
`■ 오후 8시 30분 — 마음과 관계
저녁 도입부 분위기: 하루를 마무리하며 마음과 관계를 돌아보는 분위기로 작성합니다.
도입부 예시 (매일 그대로 반복하지 말고 변형해서 사용):
- 오늘 저녁의 카드는 '○○'예요. 마음에 남아 있는 감정을 천천히 살펴볼게요.
- 하루 끝에 만난 카드는 '○○'예요. 지금 마음이 머무는 곳을 함께 들여다봐요.
- 오늘 밤 '○○' 카드가 전하는 관계의 메시지를 조용히 확인해볼까요?
...
리딩 구성:
도입부(1~2문장): 카드 이름을 자연스럽게 포함해 하루를 마무리하는 분위기로 엽니다.
1문단: 독자의 현재 감정이나 떠오르는 관계를 짧게 묻습니다. (도입부와 비슷한 내용을 반복하지 않습니다.)
...
- "오늘 아침" 또는 "오늘 저녁"이라는 표현을 같은 게시물 안에서 여러 번 반복하지 않습니다.
- 미래 결과, 재회, 연락, 상대방 마음을 확정적으로 단정하지 않습니다.
- "우주의 타이밍", "곧 연락이 옵니다" 같은 근거 없는 표현 금지.
- 도입부 다음에는 카드의 사전적 의미를 설명하지 말고 독자가 느낄 수 있는 상황이나 감정으로 자연스럽게 이어집니다.
- CTA는 게시물당 하나만 사용합니다.
- 참여형 CTA와 프로필 방문 CTA를 동시에 넣지 않습니다.
- CTA는 본문의 카드 메시지와 자연스럽게 이어져야 합니다.
- 독자가 3초 안에 답할 수 있도록 작성합니다. 개인적인 사정을 길게 설명하지 않아도 됩니다.
- "댓글로 남겨주세요"를 매번 반복하지 마세요.
- CTA 뒤에 추가 설명이나 인사말을 붙이지 마세요.
- CTA는 해시태그 바로 앞에 배치하세요.

CTA 유형을 아래 비율로 번갈아 사용하세요:
1. 선택형(40%): 두 가지 선택지 중 하나를 고르도록 작성. 예: "지금 이 관계는 '붙잡고 싶다'와 '내려놓고 싶다' 중 어느 쪽에 가까운가요?"
2. 한 단어형(20%): 독자가 자신의 상태를 한 단어로 표현. 예: "지금 가장 마음에 남는 감정을 한 단어로 표현한다면?"
3. 이모지형(15%): 이모지 하나로 반응. 예: "지금 마음을 조금 내려놓고 싶다면 🌙 하나 남겨주세요."
4. 여운형(15%): 댓글 요구 없이 마음을 돌아보는 질문. 예: "내가 붙잡고 있는 건 그 사람일까요, 그때의 마음일까요?"
5. 프로필형(10%): 자연스럽게 프로필로 안내. 예: "이 관계의 흐름을 조금 더 자세히 알고 싶다면 프로필에서 이어서 만나보세요."

오전 CTA는 오늘의 행동·선택·마음가짐과 연결하세요.
저녁 CTA는 관계·감정·기다림과 연결하고, 독자 자신의 마음을 돌아보게 하세요.

CTA 최종 점검: 본문과 연결되는가 / 3초 안에 답할 수 있는가 / CTA가 한 개인가 / 이전과 표현이 겹치지 않는가. 하나라도 충족 못하면 다시 작성하세요.

- 이모지는 게시물당 1~2개만 사용합니다.
- 마크다운 기호(**) 사용 금지.
- 해시태그는 내용에 맞는 것 2~3개만 작성하며, 반드시 모든 해시태그 앞에 #을 붙입니다. 예: #데일리타로 #오늘의타로 #타로리딩 (첫 번째 해시태그의 # 누락 금지)
- 전체 분량은 공백 포함 330~480자 이내로 작성합니다. (도입부 포함 기준)

[최종 확인] 출력 전 아래 항목을 확인하세요. 하나라도 충족하지 않으면 다시 작성합니다.
- 도입부가 1~2문장으로 짧은가?
- 카드 이름이 도입부에 자연스럽게 한 번 포함됐는가?
- 카드 이름이 본문에서 다시 반복되지 않았는가?
- 오전과 저녁의 분위기가 정확히 구분됐는가?
- 운명론적이거나 과장된 표현이 없는가?
- 도입부와 본문이 자연스럽게 연결되며 비슷한 내용을 반복하지 않는가?
- 모든 해시태그 앞에 #이 붙어 있는가?
